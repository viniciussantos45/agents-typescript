...
export const confirmHumanTool = tool(
  async ({ action, consequence, defaultConfirm = false }) => {
    print("\n");
    ...
    divider(C.yellow);
    print("\n");
    return JSON.stringify({ confirmed, comment });
  },
  {
    name: "confirm_human",
    description:
...
export const chooseHumanTool = tool(
  async ({ question, options, context }) => {
    ...
    divider(C.magenta);
    print("\n");
    return JSON.stringify({ choice: options[index], index });
  },
  {
    name: "choose_human",
    description:
...
export const reviewHumanTool = tool(
  async ({ title, content, instructions }) => {
    print("\n");
    ...
    divider(C.blue);
    print("\n");
    return JSON.stringify({ approved, feedback });
  },
  {
    name: "review_human",
    description:
...
export const notifyHumanTool = tool(
  async ({ message, level = "info", title }) => {
    const styles: Record<
...
export const waitForHumanTool = tool(
  async ({ checkpoint, summary, nextStep }) => {
    ...
    return JSON.stringify({ resumedAt, elapsed });
  },
  {
    name: "wait_for_human",
    description:
...
export const rateHumanTool = tool(
  async ({ subject, min = 1, max = 5, labels, askReason = false }) => {
    print("\n");
    ...
    divider(C.cyan);
    print("\n");
    return JSON.stringify({ rating, reason });
  },
  {
    name: "rate_human",
    description:
    ...
    schema: z.object({
      ...
      askReason: z
        ...
        .optional()
        .describe(
          "Also ask the human to explain their rating (default: false)",
        ),
    }),
  },
);
